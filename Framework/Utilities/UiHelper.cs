    ...
    class UiHelper
    {
        ...
        private static void DrawInitialRowLine(Canvas canvas, double scaleValue)
        {
            if (GrayInitialImage != null)
                _initialRowLine = GetRowLine(canvas, GrayInitialImage, LastMouseClick, scaleValue);
            else if (ColorInitialImage != null)
                _initialRowLine = GetRowLine(canvas, ColorInitialImage, LastMouseClick, scaleValue);
        }

        private static void DrawProcessedRowLine(Canvas canvas, double scaleValue)
        {
            if (GrayProcessedImage != null)
                _processedRowLine = GetRowLine(canvas, GrayProcessedImage, LastMouseClick, scaleValue);
            else if (ColorProcessedImage != null)
                _processedRowLine = GetRowLine(canvas, ColorProcessedImage, LastMouseClick, scaleValue);
        }

        private static void DrawInitialColumnLine(Canvas canvas, double scaleValue)
        {
            if (GrayInitialImage != null)
                _initialColumnLine = GetColumnLine(canvas, GrayInitialImage, LastMouseClick, scaleValue);
            else if (ColorInitialImage != null)
                _initialColumnLine = GetColumnLine(canvas, ColorInitialImage, LastMouseClick, scaleValue);
        }

        private static void DrawProcessedColumnLine(Canvas canvas, double scaleValue)
        {
            if (GrayProcessedImage != null)
                _processedColumnLine = GetColumnLine(canvas, GrayProcessedImage, LastMouseClick, scaleValue);
            else if (ColorProcessedImage != null)
                _processedColumnLine = GetColumnLine(canvas, ColorProcessedImage, LastMouseClick, scaleValue);
        }

        private static void DrawInitialSquare(Canvas canvas, double scaleValue)
        {
            if (GrayInitialImage != null || ColorInitialImage != null)
                _initialSquare = GetSquare(canvas, LastMouseClick, scaleValue);
        }
        ...
        private static void DrawProcessedSquare(Canvas canvas, double scaleValue)
        {
            if (GrayProcessedImage != null || ColorProcessedImage != null)
                _processedSquare = GetSquare(canvas, LastMouseClick, scaleValue);
        }
        #endregion
    }
}